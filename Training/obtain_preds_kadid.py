...
# JIT compile the distance calculation for a batch (PerceptNet / Original with state)
@jax.jit
...
def evaluate_dataset_predictions(state, dataset_rdy, is_baseline):
    all_dists = []
    all_mos = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Obtain per-pair predictions for KADID-trained models and upload to W&B"
    )
    parser.add_argument(
        "--tid08_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="W&B project name (e.g. Jorgvt/PerceptNet_v15)",
    )
    parser.add_argument(
        "--job_type",
        type=str,
        default="training-kadid10k",
        help="W&B jobType filter (e.g. training-kadid10k)",
    )
    parser.add_argument(
        "--run_ids",
        nargs="+",
        default=None,
        help="Specific W&B run IDs to evaluate (overrides job_type filter if provided)",
    )
    parser.add_argument(
        "--all_checkpoints",
        action="store_true",
        help="Evaluate all checkpoints (model-0, model-best, model-final) instead of only model-best",
    )
    parser.add_argument(
        "--predictions_dir",
        type=str,
        default="predictions_kadid",
    ...
    api = wandb.Api()
    if args.run_ids:
        print(f"Fetching {len(args.run_ids)} specified runs from W&B project '{args.project}'...")
        runs = [api.run(f"{args.project}/{run_id}") for run_id in args.run_ids]
    else:
        print(f"Fetching runs from W&B project '{args.project}' with jobType '{args.job_type}'...")
        runs = list(api.runs(args.project, filters={"jobType": args.job_type}, order="-created_at"))

    print(f"Total runs to evaluate: {len(runs)}")
    results = []
    ...
    for run in runs:
        ...
        checkpoint_types = (
            ["model-0", "model-best", "model-final"]
        ...
        for ckpt_name in checkpoint_types:
            ...
            if len(files) > 0:
                ...
                # Fall back to local search in case files were stored locally
                local_runs = glob.glob(
                    f"wandb/run-*-{run_id}/files/{ckpt_name}/checkpoint"
                )
                if len(local_runs) > 0:
            ...
            if not checkpoint_dir:
                print(f"Checkpoint {ckpt_name} not found for run {run_name} (ID: {run_id}). Skipping...")
                continue
            ...
            try:
                ...
                # Evaluate TID2008, TID2013 and KADIK10K
                tid08_corr, tid08_dists, tid08_mos = evaluate_dataset_predictions(
                    aligned_state, dst_tid08_rdy, is_baseline
                )
                tid13_corr, tid13_dists, tid13_mos = evaluate_dataset_predictions(
                    aligned_state, dst_tid13_rdy, is_baseline
                )
                kad_corr, kad_dists, kad_mos = evaluate_dataset_predictions(
                    aligned_state, dst_kad_rdy, is_baseline
                )

                print(f"[{run_name} | {ckpt_name}]")
                ...
                run_pred_dir = (
                    os.path.join(args.predictions_dir, run_id, ckpt_name)
                    if args.all_checkpoints
                    else os.path.join(args.predictions_dir, run_id)
                )
    ...
    if len(results) > 0:
        ...
        with open("Training/local_kadid_correlations.md", "w") as f:
            f.write(output_text)

        df.to_csv("Training/local_kadid_correlations.csv", index=False)
        ...
        print("\nNo KADID-trained runs with checkpoints were successfully evaluated.")
